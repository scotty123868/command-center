...
export const topOpportunities: Opportunity[] = [
  { name: 'RailSentry Enhancement', category: 'Workflow Automation', savings: 1_200_000, effort: 'Medium', status: 'automated', priority: 10, timeToValue: 8, confidence: 94 },
  { name: 'Predictive Maintenance (HCSS + Equipment360)', category: 'Workflow Automation', savings: 980_000, effort: 'Medium', status: 'identified', priority: 9, timeToValue: 12, confidence: 86 },
  { name: 'Project Cost Intelligence (eCMS + P6)', category: 'Workflow Automation', savings: 850_000, effort: 'High', status: 'identified', priority: 9, timeToValue: 16, confidence: 78 },
  { name: 'Field Ops AI (HCSS Field + Heavy Job)', category: 'Workflow Automation', savings: 720_000, effort: 'Medium', status: 'identified', priority: 8, timeToValue: 12, confidence: 82 },
  { name: 'Bid Intelligence (Heavy Bid)', category: 'Workflow Automation', savings: 640_000, effort: 'Low', status: 'identified', priority: 8, timeToValue: 8, confidence: 84 },
  { name: 'Crew Scheduling (MCP + HCSS)', category: 'Workflow Automation', savings: 580_000, effort: 'Medium', status: 'identified', priority: 7, timeToValue: 14, confidence: 80 },
  { name: 'Document Intelligence (Prolog + Procore)', category: 'Workflow Automation', savings: 420_000, effort: 'Low', status: 'identified', priority: 7, timeToValue: 8, confidence: 88 },
  { name: 'HSI Ultrasonic Expansion (B→A scan)', category: 'Workflow Automation', savings: 380_000, effort: 'High', status: 'automated', priority: 8, timeToValue: 18, confidence: 76 },
  { name: 'Safety Risk Prediction (HCSS Safety)', category: 'Workflow Automation', savings: 320_000, effort: 'Low', status: 'identified', priority: 6, timeToValue: 6, confidence: 90 },
...
export const recommendations: Recommendation[] = [
  {
    current: {
      ...
      users: 160,
      score: 3,
      description: 'On-premise eCMS construction ERP from Computer Guidance serving all 7 divisions with 160 named users. Handles financial management, project cost accounting, AP/AR, GL, and multi-division reporting. Strong construction-specific capabilities but relies on flat-file exchanges for inter-system integration. Month-end close creates batch conflicts with P6 sync. 90 of 250 licensed seats inactive in past 90 days. Core system — not being replaced, but needs AI middleware layer.',
    },
    recommended: {
      name: 'eCMS + AI Middleware',
      cost: 200_000,
      description: 'AI middleware layer running alongside eCMS in Herzog TX data center. Automated invoice matching using ML models trained on 5 years of eCMS financial data — reduces manual invoice processing by 65%. Project cost prediction models cross-reference eCMS actuals with P6 schedules to forecast cost overruns 4-6 weeks ahead. Runs on-prem (no cloud dependency) with lakehouse integration for cross-division financial intelligence. eCMS remains the system of record — AI layer reads and recommends, humans approve and commit.',
    },
    annualSavings: 480_000,
  },
  {
    current: {
      name: 'QMirror (MTL Systems)',
      ...
      users: 20,
      score: 1,
      description: 'QMirror batch replication tool from MTL Systems replicating AS/400 data to PC-based databases. 15-minute replication lag creates data freshness bottleneck for all downstream systems. Single point of failure — when QMirror falls behind during peak dispatch, downstream reports and dashboards show stale data. Only 20 direct users but impacts every system that consumes AS/400 data. The root cause blocker for real-time analytics across the enterprise.',
    },
    recommended: {
      name: 'Real-time CDC Pipeline (Kafka + Debezium)',
      cost: 80_000,
      description: 'On-premise Apache Kafka + Debezium change data capture pipeline replacing batch AS/400 replication with sub-second streaming. Deployed in Herzog TX data center alongside existing infrastructure. Eliminates 15-minute data lag — all downstream consumers receive changes in real-time. Enables the entire AI analytics layer: without real-time data, predictive maintenance, crew scheduling AI, and cross-division dashboards cannot function. Foundation investment that unlocks $3.2M in downstream AI savings.',
    },
  ...
  },
  {
    current: {
      ...
      users: 2_400,
      score: 7,
      description: 'HCSS Telematics tracking 800+ vehicles across all 7 divisions with real-time GPS, engine diagnostics, geofencing, and fleet management. High adoption (2,400 active users) and strong data quality. Currently used for location tracking and basic fleet management — rich diagnostic and usage pattern data not yet leveraged for predictive analytics. Equipment360 maintenance data exists in parallel but no AI correlation between telematics signals and maintenance outcomes.',
    },
    recommended: {
      name: 'HCSS Telematics + AI Overlay',
      cost: 100_000,
      description: 'AI overlay on existing HCSS Telematics + Equipment360 data. Predictive maintenance models trained on historical telematics patterns correlated with Equipment360 work orders — predict equipment failures 4-6 weeks before occurrence. Route optimization using GPS data reduces idle travel time by 15%. Anomaly detection on engine diagnostics flags emerging issues before they trigger breakdowns. Runs on-prem lakehouse compute, reads from existing HCSS APIs. No changes to HCSS configuration — pure AI enhancement layer.',
    },
    annualSavings: 380_000,
  },
  {
    current: {
      name: 'RailSentry / Tie / HSI AI Models',
      cost: 0,
      users: 45,
      score: 7,
      description: 'Three AI models already in production: RailSentry (97.1% accuracy on rail defect detection), Tie Inspection AI (89.2% on wood/concrete/composite classification), and HSI Ultrasonic AI (80.4% on B-scan defect detection). Built by internal engineering teams. Currently lacking MLOps infrastructure — no model versioning, no automated retraining, no A/B testing framework. Models deployed manually to inspection vehicles with no edge inference optimization.',
    },
    recommended: {
      name: 'RailSentry/Tie/HSI MLOps Pipeline',
      cost: 120_000,
      description: 'Enterprise MLOps pipeline for all three production AI models. Model versioning and registry using MLflow on-prem. Automated retraining triggers when model drift detected. A/B testing framework for safe model updates on inspection vehicles. Edge deployment framework for offline-capable inference on geometry cars and inspection equipment. Move HSI Ultrasonic from 80% to 95% accuracy with B→A scan expansion. Centralized model monitoring dashboard with Purview governance integration.',
    },
  ...
  },
  {
    current: {
      ...
      users: 80,
      score: 3,
      description: 'SAP Business Objects serving 80 users for dashboards, ad-hoc reporting, and data visualization across divisions. Static report generation — users must manually build queries and wait for batch-processed results. No natural language interface. Cross-division queries frequently timeout due to data volume. 80 of 160 licensed seats inactive. Reports reflect data that is already 15+ minutes stale due to QMirror replication lag.',
    },
    recommended: {
      name: 'AI Analytics Platform',
      cost: 80_000,
      description: 'AI-powered analytics platform replacing static Business Objects dashboards. Natural language query interface — users ask questions in plain English, AI generates cross-division insights from lakehouse data. Automated anomaly detection surfaces important trends without manual report building. Pre-aggregated materialized views eliminate timeout issues on cross-division queries. Runs on-prem lakehouse compute with real-time data from CDC pipeline (post-QMirror replacement). Purview-governed data access controls.',
    },
    annualSavings: 200_000,
  },
  {
    current: {
      name: 'No Unified Data Lakehouse',
      cost: 0,
      users: 0,
      score: 1,
      description: 'Data siloed across 18 applications in 2 data centers (TX/AZ). eCMS financial data disconnected from HCSS field operations. P6 project schedules not correlated with actual field production from Heavy Job. CMMS inventory records isolated from Equipment360 maintenance workflows. RailSentry inference results stored separately from operational data. QMirror batch replication creates 15-minute data lag. No ability to train cross-system ML models or generate unified analytics.',
    },
    recommended: {
      name: 'On-Prem Delta Lakehouse',
      cost: 280_000,
      description: 'On-premise Delta Lake deployed in Herzog TX data center connecting all 18 production systems: eCMS, HCSS suite (Telematics, Heavy Job, Heavy Bid, Equipment360, Safety, Field), P6, CMMS, RailSentry, MCP, Business Objects, Procore, Prolog, PMWeb, iCIMS, and QMirror replacement CDC feeds. Foundation for all AI initiatives — enables cross-system model training, unified analytics, and real-time operational intelligence. Purview governance for data access and lineage tracking. Estimated $3.2M in downstream AI savings enabled by this investment.',
    },
...
export const workflows: Workflow[] = [
  {
    name: 'P6 Schedule Optimization → Prolog Document Control',
    level: 'human-in-loop',
    currentFTEs: 18,
  ...
  {
    name: 'HCSS Field Timecard → MCP Payroll Processing',
    level: 'full',
    currentFTEs: 8,
    currentCost: 720_000,
  ...
  {
    name: 'Equipment360 PM Alert → Heavy Job Work Order',
    level: 'human-in-loop',
    currentFTEs: 12,
    currentCost: 960_000,
  ...
  {
    name: 'HCSS Safety Incident → FRA Compliance Filing',
    level: 'full',
    currentFTEs: 6,
    currentCost: 540_000,
  ...
  {
    name: 'eCMS Invoice Processing & Cost Allocation',
  ...
  {
    name: 'Heavy Bid Estimation → eCMS Project Setup',
    level: 'human-in-loop',
    currentFTEs: 14,
    currentCost: 980_000,
  ...
  {
    name: 'RailSentry Inspection → HCSS Safety Reporting',
    level: 'human-in-loop',
    currentFTEs: 20,
...
export const licenses: License[] = [
  { vendor: 'eCMS (Computer Guidance)', totalLicenses: 250, active90d: 160, inactive: 90, annualWaste: 180_000, action: 'Reclaim 90 inactive seats — restrict to active financial and project accounting users only', costPerLicense: 2_000, department: 'Finance & Project Accounting (all divisions)', lastAuditDate: '2025-10-15', trend: [82, 76, 70, 68, 65, 64], complianceRisk: false },
  { vendor: 'HCSS Suite (bundled)', totalLicenses: 2_800, active90d: 2_400, inactive: 400, annualWaste: 120_000, action: 'Reclaim 400 inactive seats across Telematics, Heavy Job, Equipment360, Safety, Field modules', costPerLicense: 300, department: 'Field Operations / All Divisions', lastAuditDate: '2026-01-10', trend: [92, 90, 88, 86, 86, 86], complianceRisk: false },
  { vendor: 'Primavera P6 (Oracle)', totalLicenses: 280, active90d: 180, inactive: 100, annualWaste: 210_000, action: 'Reclaim 100 seats — restrict to active project managers and schedulers', costPerLicense: 2_100, department: 'Project Management (HCC, HTI, HTSI, HE)', lastAuditDate: '2025-10-15', trend: [82, 76, 70, 68, 65, 64], complianceRisk: false },
  { vendor: 'Procore', totalLicenses: 300, active90d: 200, inactive: 100, annualWaste: 140_000, action: 'Reclaim 100 inactive seats — consolidate to active project teams', costPerLicense: 1_400, department: 'Construction Management (HCC, HE)', lastAuditDate: '2025-11-02', trend: [78, 72, 68, 67, 67, 67], complianceRisk: false },
  { vendor: 'Business Objects (SAP)', totalLicenses: 160, active90d: 80, inactive: 80, annualWaste: 160_000, action: 'Reclaim 80 seats — migrate to AI analytics platform in Q4', costPerLicense: 2_000, department: 'BI/Reporting (all divisions)', lastAuditDate: '2025-09-15', trend: [72, 66, 60, 55, 50, 50], complianceRisk: false },
  { vendor: 'iCIMS', totalLicenses: 80, active90d: 40, inactive: 40, annualWaste: 48_000, action: 'Reclaim 40 inactive recruiter seats — restrict to active HR team', costPerLicense: 1_200, department: 'HR / Recruiting', lastAuditDate: '2026-02-01', trend: [68, 62, 56, 52, 50, 50], complianceRisk: false },
  { vendor: 'Heavy Bid (HCSS)', totalLicenses: 200, active90d: 120, inactive: 80, annualWaste: 96_000, action: 'Reclaim 80 inactive seats — consolidate to active estimators only', costPerLicense: 1_200, department: 'Estimating (HCC, HRSI, HTI, HTSI, HE, GG)', lastAuditDate: '2025-08-20', trend: [76, 70, 64, 62, 60, 60], complianceRisk: false },
];
...
export const roiSummary = {
  techStackSavings: 3_060_000,
  workflowAutomation: 4_200_000,
  licenseRecovery: 954_000,
  implementationCosts: 1_814_000,
  netYear1: 6_400_000,
  year2Projected: 9_600_000,
};
...
const hccCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 180_000, users: 40, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Project cost accounting', 'AP/AR'] },
  { name: 'Primavera P6', category: 'Project Portfolio Mgmt', annualCost: 120_000, users: 60, score: 5, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'Medium', dependencies: ['Project scheduling', 'Resource mgmt', 'Critical path'] },
  { name: 'Procore', category: 'Construction Mgmt', annualCost: 80_000, users: 60, score: 8, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['Project mgmt', 'Quality/safety', 'BIM'] },
  { name: 'Heavy Job (HCSS)', category: 'Field Ops', annualCost: 50_000, users: 180, score: 6, integrationComplexity: 'Medium', migrationWeeks: 8, riskLevel: 'Medium', dependencies: ['Timecards', 'Production tracking', 'Budget analysis'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 70_000, users: 400, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Diagnostics', 'Geofencing'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 40_000, users: 200, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders', 'Parts inventory'] },
];
...
const hrsiCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 100_000, users: 25, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Cost accounting', 'AP/AR'] },
  { name: 'CMMS', category: 'Maintenance Mgmt', annualCost: 60_000, users: 40, score: 5, integrationComplexity: 'Medium', migrationWeeks: 10, riskLevel: 'Medium', dependencies: ['Work orders', 'PM scheduling', 'Inventory'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 45_000, users: 180, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Diagnostics', 'Fleet mgmt'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 25_000, users: 120, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders', 'Parts'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 45_000, users: 380, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'Work reports', 'PTO'] },
];
...
const hsiCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 80_000, users: 15, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Cost accounting'] },
  { name: 'CMMS', category: 'Maintenance Mgmt', annualCost: 40_000, users: 30, score: 5, integrationComplexity: 'Medium', migrationWeeks: 10, riskLevel: 'Medium', dependencies: ['Inventory records', 'PM scheduling'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 30_000, users: 80, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Vehicle diagnostics'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 20_000, users: 60, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 25_000, users: 220, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'Invoicing'] },
  { name: 'Business Objects', category: 'BI/Reporting', annualCost: 20_000, users: 10, score: 3, integrationComplexity: 'Medium', migrationWeeks: 12, riskLevel: 'Medium', dependencies: ['Dashboards', 'Ad-hoc reporting'] },
];
...
const htiCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 100_000, users: 25, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Project cost accounting'] },
  { name: 'Primavera P6', category: 'Project Portfolio Mgmt', annualCost: 80_000, users: 40, score: 5, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'Medium', dependencies: ['Project scheduling', 'Resource mgmt'] },
  { name: 'Heavy Job (HCSS)', category: 'Field Ops', annualCost: 30_000, users: 100, score: 6, integrationComplexity: 'Medium', migrationWeeks: 8, riskLevel: 'Medium', dependencies: ['Timecards', 'Production tracking'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 35_000, users: 120, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Diagnostics'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 20_000, users: 80, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 35_000, users: 310, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'Work reports'] },
];
...
const htsiCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 80_000, users: 20, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Cost accounting'] },
  { name: 'Primavera P6', category: 'Project Portfolio Mgmt', annualCost: 60_000, users: 30, score: 5, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'Medium', dependencies: ['Project scheduling', 'Resource mgmt'] },
  { name: 'PMWeb', category: 'Project Controls', annualCost: 50_000, users: 25, score: 5, integrationComplexity: 'Medium', migrationWeeks: 10, riskLevel: 'Medium', dependencies: ['Cost control', 'Document mgmt'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 30_000, users: 120, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Fleet mgmt'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 20_000, users: 80, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 50_000, users: 480, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'Invoicing'] },
];
...
const heCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 60_000, users: 10, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Cost accounting'] },
  { name: 'Primavera P6', category: 'Project Portfolio Mgmt', annualCost: 40_000, users: 15, score: 5, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'Medium', dependencies: ['Project scheduling'] },
  { name: 'CMMS', category: 'Maintenance Mgmt', annualCost: 30_000, users: 20, score: 5, integrationComplexity: 'Medium', migrationWeeks: 10, riskLevel: 'Medium', dependencies: ['Work orders', 'Inventory'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 15_000, users: 50, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Diagnostics'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 10_000, users: 40, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 15_000, users: 120, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'PTO'] },
];
...
const ggCurrentStack: CurrentTool[] = [
  { name: 'eCMS', category: 'Construction ERP', annualCost: 40_000, users: 8, score: 3, integrationComplexity: 'High', migrationWeeks: 24, riskLevel: 'High', dependencies: ['Financial mgmt', 'Cost accounting'] },
  { name: 'Heavy Bid (HCSS)', category: 'Estimating', annualCost: 15_000, users: 10, score: 6, integrationComplexity: 'Medium', migrationWeeks: 8, riskLevel: 'Medium', dependencies: ['Bid management', 'Cost tracking'] },
  { name: 'HCSS Telematics', category: 'Fleet/GPS', annualCost: 10_000, users: 30, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['GPS tracking', 'Fleet mgmt'] },
  { name: 'Equipment360 (HCSS)', category: 'Fleet Maintenance', annualCost: 8_000, users: 25, score: 7, integrationComplexity: 'Low', migrationWeeks: 4, riskLevel: 'Low', dependencies: ['PM scheduling', 'Work orders'] },
  { name: 'MCP', category: 'Payroll/Time', annualCost: 10_000, users: 90, score: 3, integrationComplexity: 'High', migrationWeeks: 16, riskLevel: 'High', dependencies: ['Timesheets', 'PTO'] },
];
...
const meridianAiAgents: AIAgent[] = [
  { name: 'RailSentry AI', subtitle: 'Rail Defect Detection', accuracy: 97.1, metric2Label: 'False Positive', metric2Value: 1.3, metric3Label: 'Detection Latency', metric3Value: '0.4s', overrideRate: 5.7, confidenceThreshold: 0.92, status: 'active', lastmileAgentId: 'railsentry-ai' },
  { name: 'Tie Inspection AI', subtitle: 'Tie Classification', accuracy: 89.2, metric2Label: 'Misclassification', metric2Value: 4.2, metric3Label: 'Classification Speed', metric3Value: '0.6s', overrideRate: 12.8, confidenceThreshold: 0.84, status: 'piloting', lastmileAgentId: 'tie-inspection-ai' },
  { name: 'HSI Ultrasonic AI', subtitle: 'B-scan Defect Detection', accuracy: 80.4, metric2Label: 'False Negative', metric2Value: 6.8, metric3Label: 'Scan Analysis', metric3Value: '1.2s', overrideRate: 18.4, confidenceThreshold: 0.76, status: 'piloting', lastmileAgentId: 'hsi-ultrasonic-ai' },
  { name: 'eCMS Invoice Intelligence', subtitle: 'Invoice Matching', accuracy: 94.0, metric2Label: 'Match Error', metric2Value: 2.4, metric3Label: 'Processing Time', metric3Value: '3.1s', overrideRate: 8.6, confidenceThreshold: 0.88, status: 'planned', lastmileAgentId: 'ecms-invoice-intelligence' },
  { name: 'HCSS Fleet Predictor', subtitle: 'Predictive Maintenance', accuracy: 91.2, metric2Label: 'False Alarm Rate', metric2Value: 3.8, metric3Label: 'Prediction Lead', metric3Value: '4.6 weeks', overrideRate: 10.4, confidenceThreshold: 0.85, status: 'planned', lastmileAgentId: 'hcss-fleet-predictor' },
  { name: 'P6 Schedule Optimizer', subtitle: 'AI Scheduling', accuracy: 87.6, metric2Label: 'Schedule Slip', metric2Value: 5.1, metric3Label: 'Optimization Time', metric3Value: '8.2 min', overrideRate: 15.2, confidenceThreshold: 0.81, status: 'piloting', lastmileAgentId: 'p6-schedule-optimizer' },
  { name: 'Heavy Bid Analyst', subtitle: 'Bid Intelligence', accuracy: 84.3, metric2Label: 'Bid Variance', metric2Value: 6.4, metric3Label: 'Analysis Time', metric3Value: '2.4 hrs', overrideRate: 19.1, confidenceThreshold: 0.78, status: 'planned', lastmileAgentId: 'heavy-bid-analyst' },
  { name: 'Crew Dispatch AI', subtitle: 'MCP + HCSS Scheduling', accuracy: 86.1, metric2Label: 'Dispatch Error', metric2Value: 5.8, metric3Label: 'Schedule Gen Time', metric3Value: '6.4 min', overrideRate: 16.7, confidenceThreshold: 0.80, status: 'planned', lastmileAgentId: 'crew-dispatch-ai' },
];
...
const hccDataSources: IntegrationDataSource[] = [
  ...
  { system: 'Procore', division: 'Construction Mgmt', recordsAnalyzed: '890 project documents', coverage: 87, status: 'Complete' },
  { system: 'Heavy Job (HCSS)', division: 'Field Ops', recordsAnalyzed: '18,420 daily records', coverage: 92, status: 'Complete' },
  { system: 'HCSS Telematics', division: 'Fleet Operations', recordsAnalyzed: '400 vehicles tracked', coverage: 96, status: 'Complete' },
  { system: 'Equipment360 (HCSS)', division: 'Fleet Maintenance', recordsAnalyzed: '4,200 work orders', coverage: 89, status: 'Complete' },
];
...
const hrsiDataSources: IntegrationDataSource[] = [
  ...
  { system: 'CMMS', division: 'Maintenance', recordsAnalyzed: '12,400 repair records', coverage: 83, status: 'In Progress' },
  { system: 'HCSS Telematics', division: 'Vehicle Telematics', recordsAnalyzed: '180 vehicles tracked', coverage: 88, status: 'Complete' },
  { system: 'Equipment360 (HCSS)', division: 'Fleet Maintenance', recordsAnalyzed: '2,800 work orders', coverage: 85, status: 'Complete' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '380 employees', coverage: 100, status: 'Complete' },
];

const hsiDataSources: IntegrationDataSource[] = [
  { system: 'eCMS', division: 'HSI Finance', recordsAnalyzed: '2,100 financial records', coverage: 90, status: 'Complete' },
  { system: 'CMMS', division: 'Maintenance', recordsAnalyzed: '3,600 inspection records', coverage: 85, status: 'In Progress' },
  { system: 'HCSS Telematics', division: 'Inspection Fleet', recordsAnalyzed: '80 vehicles tracked', coverage: 94, status: 'Complete' },
  { system: 'Ultrasonic Sensor Archive', division: 'Defect Imaging', recordsAnalyzed: '240,000 images', coverage: 89, status: 'In Progress' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '220 employees', coverage: 100, status: 'Complete' },
];
...
const htiDataSources: IntegrationDataSource[] = [
  ...
  { system: 'Primavera P6', division: 'Project Scheduling', recordsAnalyzed: '180 signal projects', coverage: 91, status: 'Complete' },
  { system: 'Heavy Job (HCSS)', division: 'Field Ops', recordsAnalyzed: '8,400 daily records', coverage: 86, status: 'In Progress' },
  { system: 'HCSS Telematics', division: 'Fleet Operations', recordsAnalyzed: '120 vehicles tracked', coverage: 92, status: 'Complete' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '310 employees', coverage: 100, status: 'Complete' },
];

const htsiDataSources: IntegrationDataSource[] = [
  { system: 'eCMS', division: 'HTSI Finance', recordsAnalyzed: '3,400 financial records', coverage: 90, status: 'Complete' },
  { system: 'Primavera P6', division: 'Project Scheduling', recordsAnalyzed: '140 transit projects', coverage: 92, status: 'Complete' },
  { system: 'PMWeb', division: 'Project Controls', recordsAnalyzed: '2,800 cost records', coverage: 88, status: 'Complete' },
  { system: 'HCSS Telematics', division: 'Vehicle Telematics', recordsAnalyzed: '120 vehicles tracked', coverage: 92, status: 'Complete' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '480 employees', coverage: 100, status: 'Complete' },
];
...
const heDataSources: IntegrationDataSource[] = [
  ...
  { system: 'Primavera P6', division: 'Project Scheduling', recordsAnalyzed: '45 energy projects', coverage: 88, status: 'Complete' },
  { system: 'CMMS', division: 'Maintenance', recordsAnalyzed: '2,400 work orders', coverage: 82, status: 'In Progress' },
  { system: 'HCSS Telematics', division: 'Fleet Operations', recordsAnalyzed: '50 vehicles tracked', coverage: 90, status: 'Complete' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '120 employees', coverage: 100, status: 'Complete' },
];
...
const ggDataSources: IntegrationDataSource[] = [
  ...
  { system: 'Heavy Bid (HCSS)', division: 'Estimating', recordsAnalyzed: '420 bid records', coverage: 86, status: 'Complete' },
  { system: 'HCSS Telematics', division: 'Fleet Operations', recordsAnalyzed: '30 vehicles tracked', coverage: 88, status: 'Complete' },
  { system: 'Equipment360 (HCSS)', division: 'Fleet Maintenance', recordsAnalyzed: '800 work orders', coverage: 82, status: 'In Progress' },
  { system: 'MCP', division: 'Workforce', recordsAnalyzed: '90 employees', coverage: 100, status: 'Complete' },
];

const meridianDataSources: IntegrationDataSource[] = [
  { system: 'eCMS (Computer Guidance)', division: 'Enterprise ERP', recordsAnalyzed: '24,847 work orders', coverage: 73, status: 'In Progress' },
  { system: 'HCSS Telematics', division: 'Fleet Operations', recordsAnalyzed: '800+ vehicles', coverage: 94, status: 'Complete' },
  { system: 'Primavera P6', division: 'Project Portfolio', recordsAnalyzed: '280 active projects', coverage: 88, status: 'Complete' },
  { system: 'Heavy Job (HCSS)', division: 'Field Operations', recordsAnalyzed: 'Daily production data', coverage: 91, status: 'Complete' },
  { system: 'CMMS', division: 'Maintenance', recordsAnalyzed: 'Inventory records', coverage: 78, status: 'In Progress' },
  { system: 'QMirror/AS400', division: 'Legacy Replication', recordsAnalyzed: 'Legacy replication', coverage: 62, status: 'Pending Access' },
];
...
const hccVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HCC)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:14:00Z', note: 'Batch delays during month-end close' },
  { name: 'Primavera P6', status: 'green', uptime: 99.5, latency: 180, lastChecked: '2026-03-27T08:12:00Z' },
  { name: 'Procore', status: 'green', uptime: 99.97, latency: 45, lastChecked: '2026-03-27T08:10:00Z' },
  { name: 'Heavy Job (HCSS)', status: 'green', uptime: 99.6, latency: 72, lastChecked: '2026-03-27T08:08:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:15:00Z' },
  { name: 'Equipment360 (HCSS)', status: 'green', uptime: 99.91, latency: 58, lastChecked: '2026-03-27T08:13:00Z' },
];

const hrsiVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HRSI)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T07:58:00Z', note: 'On-prem batch delays' },
  { name: 'CMMS', status: 'green', uptime: 99.4, latency: 86, lastChecked: '2026-03-27T08:02:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:04:00Z' },
  { name: 'Equipment360 (HCSS)', status: 'green', uptime: 99.91, latency: 58, lastChecked: '2026-03-27T08:06:00Z' },
  { name: 'MCP (HRSI)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T07:56:00Z' },
];

const hsiVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HSI)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:18:00Z', note: 'On-prem batch delays' },
  { name: 'CMMS', status: 'green', uptime: 99.4, latency: 86, lastChecked: '2026-03-27T08:16:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:20:00Z' },
  { name: 'Ultrasonic Sensor Archive', status: 'yellow', uptime: 96.4, latency: 428, lastChecked: '2026-03-27T08:14:00Z', note: 'High latency on large image batch retrieval — internal ultrasonic tools' },
  { name: 'MCP (HSI)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T08:22:00Z' },
];

const htiVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HTI)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:24:00Z', note: 'On-prem batch delays' },
  { name: 'Primavera P6', status: 'green', uptime: 99.5, latency: 180, lastChecked: '2026-03-27T08:22:00Z' },
  { name: 'Heavy Job (HCSS)', status: 'green', uptime: 99.6, latency: 72, lastChecked: '2026-03-27T08:20:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:18:00Z' },
  { name: 'MCP (HTI)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T08:26:00Z' },
];

const htsiVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HTSI)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:28:00Z', note: 'On-prem batch delays' },
  { name: 'Primavera P6', status: 'green', uptime: 99.5, latency: 180, lastChecked: '2026-03-27T08:30:00Z' },
  { name: 'PMWeb', status: 'green', uptime: 99.3, latency: 92, lastChecked: '2026-03-27T08:26:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:24:00Z' },
  { name: 'MCP (HTSI)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T08:22:00Z' },
];

const heVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (HE)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:32:00Z', note: 'On-prem batch delays' },
  { name: 'Primavera P6', status: 'green', uptime: 99.5, latency: 180, lastChecked: '2026-03-27T08:30:00Z' },
  { name: 'CMMS', status: 'green', uptime: 99.4, latency: 86, lastChecked: '2026-03-27T08:34:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:28:00Z' },
  { name: 'MCP (HE)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T08:36:00Z' },
];

const ggVendorHealth: IntegrationVendorHealth[] = [
  { name: 'eCMS (GG)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:38:00Z', note: 'On-prem batch delays' },
  { name: 'Heavy Bid (HCSS)', status: 'green', uptime: 99.6, latency: 72, lastChecked: '2026-03-27T08:36:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:40:00Z' },
  { name: 'Equipment360 (HCSS)', status: 'green', uptime: 99.91, latency: 58, lastChecked: '2026-03-27T08:42:00Z' },
  { name: 'MCP (GG)', status: 'green', uptime: 99.3, latency: 44, lastChecked: '2026-03-27T08:44:00Z' },
];

const meridianVendorHealth: IntegrationVendorHealth[] = [
  { name: 'Procore', status: 'green', uptime: 99.97, latency: 45, lastChecked: '2026-03-27T08:46:00Z' },
  { name: 'HCSS Telematics', status: 'green', uptime: 99.94, latency: 62, lastChecked: '2026-03-27T08:44:00Z' },
  { name: 'Primavera P6', status: 'green', uptime: 99.5, latency: 180, lastChecked: '2026-03-27T08:48:00Z' },
  { name: 'Equipment360 (HCSS)', status: 'green', uptime: 99.91, latency: 58, lastChecked: '2026-03-27T08:50:00Z' },
  { name: 'eCMS (Computer Guidance)', status: 'yellow', uptime: 99.2, latency: 340, lastChecked: '2026-03-27T08:42:00Z', note: 'On-prem — periodic batch delays during month-end close' },
  { name: 'QMirror (MTL Systems)', status: 'yellow', uptime: 98.1, latency: 2400, lastChecked: '2026-03-27T08:40:00Z', note: 'AS/400 batch replication — 15-min data lag' },
];
...
const hccFailureModes: IntegrationFailureMode[] = [
  ...
  { vendor: 'Procore', scenario: 'Large document upload timeout on BIM files (>500MB)', recovery: 'Chunked upload with resume capability and checksum validation', status: 'Passing' },
  ...
  { vendor: 'Heavy Job (HCSS)', scenario: 'Field sync failure when cellular coverage unavailable', recovery: 'Offline queue with store-and-forward on reconnect', status: 'Passing' },
];

const hrsiFailureModes: IntegrationFailureMode[] = [
  { vendor: 'eCMS (HRSI)', scenario: 'Flat-file export failure during inter-system data exchange', recovery: 'Retry queue with validation and manual fallback', status: 'Needs Attention' },
  { vendor: 'CMMS', scenario: 'Slow query timeout on historical repair record aggregation', recovery: 'Pre-computed materialized views refreshed nightly', status: 'Needs Attention' },
  { vendor: 'HCSS Telematics', scenario: 'Cellular dead zones causing telemetry gaps on rural rail corridors', recovery: 'On-device buffering with store-and-forward sync', status: 'Passing' },
...
const hsiFailureModes: IntegrationFailureMode[] = [
  { vendor: 'Ultrasonic Sensor Archive', scenario: 'Image batch upload fails on corrupted sensor frames from internal ultrasonic tools', recovery: 'Frame-level validation with corrupt frame isolation and re-scan flag', status: 'Passing' },
  { vendor: 'eCMS (HSI)', scenario: 'Flat-file exchange failure with CMMS inventory records', recovery: 'Retry with validation and manual reconciliation workflow', status: 'Needs Attention' },
  { vendor: 'HCSS Telematics', scenario: 'GPS dropout during inspection runs in remote corridors', recovery: 'Offline queue with sync-on-reconnect', status: 'Passing' },
  { vendor: 'CMMS', scenario: 'Inventory record sync lag with Equipment360 work orders', recovery: 'Automated reconciliation with variance threshold alerts', status: 'Passing' },
];

const htiFailureModes: IntegrationFailureMode[] = [
  { vendor: 'eCMS (HTI)', scenario: 'Project cost sync failure between eCMS and P6 during concurrent updates', recovery: 'Retry with conflict resolution and manual reconciliation', status: 'Needs Attention' },
  { vendor: 'Primavera P6', scenario: 'API token expiration during multi-project schedule sync', recovery: 'Auto-refresh token with 15-min buffer before expiry', status: 'Passing' },
  { vendor: 'Heavy Job (HCSS)', scenario: 'Field sync failure in remote signal installation locations', recovery: 'Offline queue with store-and-forward on reconnect', status: 'Passing' },
  { vendor: 'HCSS Telematics', scenario: 'GPS dropout during signal testing in remote corridors', recovery: 'Dead reckoning interpolation with last-known-good coordinates', status: 'Passing' },
];

const htsiFailureModes: IntegrationFailureMode[] = [
  { vendor: 'eCMS (HTSI)', scenario: 'Project cost allocation mismatch during multi-project transit operations', recovery: 'Automated reconciliation with variance threshold alerts', status: 'Passing' },
  { vendor: 'PMWeb', scenario: 'Document sync failure between PMWeb and Primavera P6 project controls', recovery: 'Retry queue with validation and manual fallback', status: 'Needs Attention' },
  { vendor: 'HCSS Telematics', scenario: 'Vehicle health telemetry flood during fleet-wide diagnostic cycle', recovery: 'Rate limiter with priority queuing for critical health alerts', status: 'Passing' },
  { vendor: 'MCP (HTSI)', scenario: 'Timecard sync delay during peak bi-weekly payroll processing', recovery: 'Retry with 3x exponential backoff and manual override alert', status: 'Passing' },
];

const heFailureModes: IntegrationFailureMode[] = [
  { vendor: 'eCMS (HE)', scenario: 'Month-end close conflict with CMMS maintenance cost sync', recovery: 'Scheduled maintenance window with read-only mode during close', status: 'Passing' },
  { vendor: 'Primavera P6', scenario: 'Schedule sync failure during concurrent multi-project updates', recovery: 'Retry with conflict detection and manual merge', status: 'Passing' },
  { vendor: 'CMMS', scenario: 'Work order sync lag between CMMS and Equipment360', recovery: 'Automated reconciliation with variance threshold alerts', status: 'Needs Attention' },
  { vendor: 'HCSS Telematics', scenario: 'GPS dropout in remote energy project locations', recovery: 'Offline queue with store-and-forward sync on reconnect', status: 'Passing' },
];
...
const ggFailureModes: IntegrationFailureMode[] = [
  ...
  { vendor: 'Heavy Bid (HCSS)', scenario: 'Bid data sync failure during concurrent estimating sessions', recovery: 'Pessimistic locking with merge conflict resolution', status: 'Passing' },
  { vendor: 'HCSS Telematics', scenario: 'GPS signal loss at remote environmental monitoring sites', recovery: 'Offline queue with store-and-forward sync', status: 'Passing' },
  { vendor: 'MCP (GG)', scenario: 'Timecard validation failure for field crew in remote locations', recovery: 'Offline submission with supervisor approval on reconnect', status: 'Needs Attention' },
];

const meridianFailureModes: IntegrationFailureMode[] = [
  { vendor: 'QMirror', scenario: 'Replication lag during peak dispatch — downstream reports show stale data', recovery: 'Failover to direct AS/400 query via ODBC bridge', status: 'Needs Attention' },
  { vendor: 'eCMS', scenario: 'Month-end batch conflict with P6 sync — financial close locks prevent real-time reads', recovery: 'Scheduled maintenance window with read from reporting replica during close', status: 'Passing' },
  { vendor: 'HCSS Telematics', scenario: 'GPS dropout in rural rail corridors with poor cellular coverage', recovery: 'Offline queue with store-and-forward sync-on-reconnect', status: 'Passing' },
  { vendor: 'Business Objects', scenario: 'Report timeout on cross-division queries exceeding 10-minute threshold', recovery: 'Pre-aggregated materialized views refreshed nightly for cross-division reports', status: 'Needs Attention' },
];
...
const hccMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Fleet Asset Discovery', description: 'Inventory all 400+ fleet vehicles via HCSS Telematics and Equipment360, map GPS-tracked assets across HCC construction sites and rail corridors.' },
  { number: 2, title: 'Field Workflow Mapping', description: 'Map 18 critical construction workflows from Heavy Bid estimation through eCMS project closeout, identifying manual handoffs between P6, Procore, and Heavy Job.' },
  { number: 3, title: 'Construction Tech Audit', description: 'Assess eCMS, Primavera P6, Procore, Heavy Job, HCSS Telematics, and Equipment360 for AI enhancement readiness and integration maturity.' },
  { number: 4, title: 'Project ROI Modeling', description: 'Model AI-driven savings across eCMS invoice automation, P6 schedule optimization, HCSS predictive maintenance, and Heavy Bid intelligence.' },
];

const hrsiMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Equipment Registry Scan', description: 'Catalog all railroad maintenance equipment via HCSS Telematics and Equipment360, map CMMS inventory records across HRSI operations.' },
  { number: 2, title: 'Maintenance Workflow Analysis', description: 'Analyze 12 core maintenance workflows from CMMS work order through MCP payroll, mapping data handoffs between eCMS and field systems.' },
  { number: 3, title: 'Railroad Tech Stack Audit', description: 'Evaluate eCMS, CMMS, HCSS Telematics, Equipment360, and MCP for AI enhancement readiness and data completeness.' },
  { number: 4, title: 'Service ROI Modeling', description: 'Quantify predictive maintenance savings from HCSS + Equipment360 data, crew optimization gains from MCP + HCSS scheduling intelligence.' },
];

const hsiMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Sensor Data Inventory', description: 'Catalog 240K+ ultrasonic images from internal ultrasonic tools, HCSS Telematics inspection fleet data, and CMMS inventory records.' },
  { number: 2, title: 'Inspection Workflow Analysis', description: 'Map end-to-end rail inspection process from route planning through HCSS Safety reporting, identifying RailSentry AI enhancement points.' },
  { number: 3, title: 'Testing Platform Audit', description: 'Assess eCMS, CMMS, HCSS Telematics, and internal ultrasonic tools for MLOps pipeline readiness and B→A scan expansion capability.' },
  { number: 4, title: 'Compliance ROI Modeling', description: 'Model RailSentry MLOps pipeline, HSI Ultrasonic AI expansion (B→A scan), and Tie Inspection edge deployment savings.' },
];

const htiMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Signal System Inventory', description: 'Document all signal installations via eCMS project records, P6 schedules, and HCSS Telematics fleet tracking across HTI territory.' },
  { number: 2, title: 'PTC Workflow Mapping', description: 'Map signal installation from Heavy Job field ops through eCMS cost accounting, identifying P6 schedule optimization opportunities.' },
  { number: 3, title: 'Signal Tech Stack Audit', description: 'Evaluate eCMS, P6, Heavy Job, HCSS Telematics, Equipment360, and MCP for AI enhancement and cross-system integration readiness.' },
  { number: 4, title: 'Infrastructure ROI Modeling', description: 'Quantify P6 schedule optimization, Heavy Job field ops AI, and HCSS predictive maintenance savings for HTI\'s 310-person team.' },
];

const htsiMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Transit Operations Analysis', description: 'Analyze transit project data via eCMS, P6 schedules, and PMWeb project controls across HTSI operations.' },
  { number: 2, title: 'Transit Schedule Mapping', description: 'Map scheduling and crew rostering workflows from P6 project planning through MCP payroll processing and HCSS Telematics fleet tracking.' },
  { number: 3, title: 'Operations Platform Audit', description: 'Assess eCMS, P6, PMWeb, HCSS Telematics, Equipment360, and MCP for AI-powered transit operations optimization.' },
  { number: 4, title: 'Service ROI Modeling', description: 'Model P6 schedule optimization, MCP crew scheduling AI, and Equipment360 predictive maintenance savings for HTSI\'s 480-employee operation.' },
];

const heMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Energy Asset Discovery', description: 'Inventory energy infrastructure assets via eCMS project records, HCSS Telematics fleet tracking, and CMMS maintenance records.' },
  { number: 2, title: 'Energy Workflow Mapping', description: 'Map energy project workflows from P6 scheduling through eCMS cost accounting, identifying Equipment360 maintenance optimization opportunities.' },
  { number: 3, title: 'Infrastructure Tech Audit', description: 'Evaluate eCMS, P6, CMMS, HCSS Telematics, Equipment360, and MCP for AI enhancement and predictive maintenance readiness.' },
  { number: 4, title: 'Energy ROI Modeling', description: 'Quantify P6 schedule optimization, HCSS predictive maintenance, and eCMS project cost intelligence savings for HE\'s 120-person team.' },
];

const ggMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'Environmental Asset Discovery', description: 'Catalog environmental project data via eCMS, Heavy Bid estimation records, and HCSS Telematics fleet tracking across IC Environmental operations.' },
  { number: 2, title: 'Environmental Workflow Mapping', description: 'Map environmental project workflows from Heavy Bid estimation through eCMS cost accounting and HCSS field operations tracking.' },
  { number: 3, title: 'Monitoring Tech Audit', description: 'Assess eCMS, Heavy Bid, HCSS Telematics, Equipment360, and MCP for AI enhancement and environmental compliance automation readiness.' },
  { number: 4, title: 'Remediation ROI Modeling', description: 'Model Heavy Bid intelligence savings, eCMS cost optimization, and Equipment360 predictive maintenance gains for GG\'s 90-person team.' },
];

const meridianMethodologySteps: IntegrationMethodologyStep[] = [
  { number: 1, title: 'License Discovery', description: 'Comprehensive audit of 18 production apps across all 7 divisions — eCMS, HCSS suite, P6, Procore, Business Objects, iCIMS, Heavy Bid, MCP, QMirror — identify unused seats across 2,800 employees.' },
  { number: 2, title: 'Workflow Mapping', description: 'Map 62 critical workflows across rail construction, testing, signals, transit, energy, and environmental divisions — reference real system handoffs (eCMS → P6, HCSS → MCP, Heavy Bid → eCMS).' },
  { number: 3, title: 'Tech Stack Health', description: 'Assess the full enterprise tech stack — eCMS, HCSS Telematics, P6, Procore, MCP, QMirror/AS400, Business Objects, CMMS — for AI enhancement readiness and integration maturity.' },
  { number: 4, title: 'ROI Modeling', description: 'Model Year 1 savings of $6.4M across AI enhancement (not replacement) of existing systems, license reclamation, and data foundation investments enabling $3.2M in downstream AI savings.' },
];
