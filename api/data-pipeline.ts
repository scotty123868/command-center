...
function generatePipelineStatus(): PipelineStatus {
  ...
  const sources: PipelineSource[] = [
    ...
    {
      id: 'hcss-suite',
      name: 'HCSS Suite (Heavy Job + Equipment360)',
      type: 'database',
      status: Math.random() > 0.9 ? 'stale' : 'batch',
      recordsIngested24h: jitter(2100, 300),
      lastIngestion: new Date(Date.now() - 3600000 * 2 - Math.random() * 3600000).toISOString(),
      freshness: 'hourly',
      schemaValid: Math.random() > 0.02,
      errorRate: 0.08,
